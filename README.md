...
#### To add ILA Core for debugging (vivado)
  - **Mark Signals Debug**
    ...
    ```
    Note: Only the signal running on independent clock will be added to the ILA and the ILA is generated with that clock as the sampling clock. Errors may occur if the signals´ driving clock is not independent. For each signal the clock ports can be mentioned as an attribute so for example for cincoranch:
    
    ```
    (* dont_touch="true", mark_debug="true", MARK_DEBUG_CLOCK="clk_mmcm/inst/clk_out1" *)  
    ```
  - **Use the `--ila` option flag on protosyn command** 
